...
function getCarInfoByIndex(inventory, index) {
  for (let k = 0; k < inventory.length; k++) {
    if(inventory[k].id === (index + 1)) {
      return `This is a ${inventory[k].car_make} ${inventory[k].car_model}`;
    }
  }
}
...
function getModelYears(inventory) {
  let years = [];
  for (let k = 0; k < inventory.length; k++) {
    years.push(inventory[k].car_year);
  }
  return years;
}
...
function getOlderCars(inventory, max_year) {
  let old_cars = [];
  for (let k = 0; k < inventory.length; k++) {
    if (inventory[k].car_year <= max_year) {
      old_cars.push(inventory[k]);
    }
  }
  return old_cars;
}
...
function getGermanCars(inventory) {
  let german_cars = [];
  for (let k = 0; k < inventory.length; k++) {
    switch (inventory[k].car_make) {
      case 'Audi':
        german_cars.push(inventory[k]);
        break;
      case 'Mercedes-Benz':
        german_cars.push(inventory[k]);
        break;
